...
public class WuziqiPanel extends View {
    ...
    private float ratioPieceOfLineHeight = 3 * 1.0f / 4;//四分之三的比例

    //坐标集合
    private boolean mIsWhite =true;//白棋先手，或者当前白棋
    private ArrayList<Point> mWhiteArray = new ArrayList<>();
    private ArrayList<Point> mBlackArray = new ArrayList<>();

    private boolean mIsGameOver;
    private boolean ismIsWhiteWiner;

    public WuziqiPanel(Context context, AttributeSet attrs){
        super(context,attrs);
        setBackgroundColor(0x4483848e);
        init();
    }
    private void init(){
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mPanekWidth = w;
        mLineHeight = mPanekWidth * 1.0f / MAX_LINE;
...
//        return true;
    ...
    private void checkGameOver(){
        ...
        if(whiteWin || blackWin){
            mIsGameOver = true;
            ismIsWhiteWiner = whiteWin;
            String text =ismIsWhiteWiner ? "白棋胜利" : "黑棋胜利";
